...
// 定義信號量屬性
osSemaphoreDef(BinarySem01);  // 定義信號量名稱
...
int main(void)
{
  ...
  BSP_ACCELERO_Init();
  //float32_t bufferX[64];
  //float32_t bufferY[64];
  //float32_t bufferZ[64];
  /* USER CODE END 2 */
  ...
  ** ------------------------------------------------------------------- */
  ...
  /*
   for (i = 0; i < TEST_LENGTH_SAMPLES; i++) {
	   printf("Input: %d\n", (int)(testInput_f32_1kHz_15kHz[i]*1000));
   }
...
void StartTask02(void const * argument)
{
  ...
  for(;;)
  {
	  ...
	  float32_t  *outputF32Y, *outputF32Z;
	  ...
	  for(i=0; i < numBlocks; i++)
	  {
		 arm_fir_f32(&S, &bufferY[0] + (i * blockSize), outputF32Y + (i * blockSize), blockSize);
	  }
	  ...
	  for(i=0; i < numBlocks; i++)
	  {
		 arm_fir_f32(&S, &bufferZ[0] + (i * blockSize), outputF32Z + (i * blockSize), blockSize);
	  }

	  for (int i = 0; i < 64; i++) {
		bufferx_int[i] = (int)outputF32[i];
		buffery_int[i] = (int)outputF32Y[i];
		bufferz_int[i] = (int)outputF32Z[i];
	  }*/

	  //ACC_DATA_Update(bufferx_int, buffery_int, bufferz_int);
	  //osDelay(100);
  }
