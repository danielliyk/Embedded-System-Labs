...
/* USER CODE BEGIN PV */
extern uint16_t current_sampling_freq;
/* USER CODE END PV */
...
/**
  ...
  * @retval int
  */
extern uint16_t freq ;  // Default frequency (1 Hz)
...
void StartTask03(void const * argument) // acc piopiopio
{
  /* USER CODE BEGIN StartTask03 */
  /* Infinite loop */
	printf("Task03 started.\n");
	...
	while (1) {
		...
		tBleStatus ret = pio_Update(buffer, sizeof(buffer));

		// Send the data over BLE
		//tBleStatus ret = Acc_Update(pDataXYZ);

		//tBleStatus ret = aci_gatt_update_char_value(AccServHandle, AccCharHandle, 0, sizeof(buffer), (uint8_t *)buffer);
		...
		// Delay based on the sampling frequency
		//vTaskDelay(getfreq());
		printf("%d", freq);
